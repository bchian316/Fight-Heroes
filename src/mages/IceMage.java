...
public class IceMage extends Mage {
    ...
    public ArrayList<Projectile> createProjectiles(double x, double y, double targetX, double targetY) {
        ...
        for (int i = -3; i <= 3; i++) {
            newProjs.add(new Projectile(x, y, Tools.getAngle(x, y, targetX, targetY) + Math.toRadians(i * 12),
    ...
    public ArrayList<Projectile> special(Player p, double targetX, double targetY) {
        ...
        for (int i = -1; i <= 1; i++) {
            newProjs.add(new Projectile(p.getCenterX(), p.getCenterY(),
                    Tools.getAngle(p.getCenterX(), p.getCenterY(), targetX, targetY) + i*Math.toRadians(25),
