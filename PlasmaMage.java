...
public class PlasmaMage extends PlayerType {
    public PlasmaMage() {
        super("Plasma Mage", 50, 100, 5, 750, new AttackStats(5, 20, 10, 150, new Color(255, 115, 115),
    ...
    public ArrayList<Projectile> createMoreProjectiles(double x, double y, double targetX, double targetY, AttackStats splitStats) {
        ArrayList<Projectile> newProjs = new ArrayList<>();
        for (int i = 0; i < 6; i++) {
            newProjs.add(new Projectile(x, y, Game.getAngle(x, y, targetX, targetY) + Math.toRadians(i*60.0), splitStats));
        }
        return newProjs;
    }
